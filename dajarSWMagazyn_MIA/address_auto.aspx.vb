...
Partial Public Class address_auto
    ...
    Protected Sub DDLPolka_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DDLPolka.SelectedIndexChanged
        Using conn As New OracleConnection(dajarSWMagazyn_MIA.MyFunction.connString)
            conn.Open()
            Dim hala As String = DDLHala.SelectedValue.ToString
            Dim rzad As String = DDLRzad.SelectedValue.ToString
    ...
    Protected Sub RefreshDDLPolka(ByRef ddlObiekt As DropDownList, ByVal hala As String, ByVal rzad As String, ByVal regal As String)
        Using conn As New OracleConnection(dajarSWMagazyn_MIA.MyFunction.connString)
            ...
            If hala <> "" And rzad <> "" And regal <> "" Then
                ...
                ddlObiekt.SelectedValue = dajarSWMagazyn_MIA.MyFunction.GetStringFromSqlExp("select min(distinct polka) from dp_swm_mia_adresy where hala='" & hala & "' and rzad='" & rzad & "' and regal='" & regal & "' and is_active='1' order by polka", conn)
            Else
                Dim znak As String = ""
                For i = 0 To 99
                    znak = i.ToString().PadLeft(2, "0")
        ...
        Using conn As New OracleConnection(dajarSWMagazyn_MIA.MyFunction.connString)
            ...
            If Session("mytyp_oper") = "W" Then
                ...
                If czy_istnieje <> "0" Then
                    ...
                    PanelMagazyn.Visible = False
                Else
                    Dim filename As String = "\sql\automag_pkt1.sql"
                    ...
                    filename = "\sql\automag_pkt2.sql"
                    fileInput = New StreamReader(dajarSWMagazyn_MIA.MyFunction.networkMainPath & filename, System.Text.Encoding.GetEncoding(1250))
                    bufor = fileInput.ReadToEnd
                    fileInput.Close()
                    Dim sql_2 As String = bufor

                    filename = "\sql\automag_pkt3.sql"
                    fileInput = New StreamReader(dajarSWMagazyn_MIA.MyFunction.networkMainPath & filename, System.Text.Encoding.GetEncoding(1250))
                    bufor = fileInput.ReadToEnd
                    fileInput.Close()
                    Dim sql_3 As String = bufor


                    If filtr <> "" Then
                        sql_1 &= " and (w3.skrot like '" & filtr & "%')"
                        sql_2 &= " and (w3.skrot like '" & filtr & "%')"
                        sql_3 &= " and (w3.skrot like '" & filtr & "%')"
                    End If

                    sql_1 &= " order by w3.skrot asc"
                    sql_2 &= " order by w3.skrot asc"
                    sql_3 &= " order by w3.skrot asc"

                    Dim result As String = dajarSWMagazyn_MIA.MyFunction.GetStringFromSqlExp(sql_1, conn)
                    If result.Length > 0 Then
                        sqlexp = sql_1
                        Session("mm_sql") = 1
                        Label10.Text = "ZAKŁADKA AUTOMAGAZYNOWANIE TOWARU WARIANT 1"
                    Else
                        result = dajarSWMagazyn_MIA.MyFunction.GetStringFromSqlExp(sql_2, conn)
                        If result.Length > 0 Then
                            sqlexp = sql_2
                            Session("mm_sql") = 2
                            Label10.Text = "ZAKŁADKA AUTOMAGAZYNOWANIE TOWARU WARIANT 2"
                        Else
                            result = dajarSWMagazyn_MIA.MyFunction.GetStringFromSqlExp(sql_3, conn)
                            If result.Length > 0 Then
                                sqlexp = sql_3
                                Session("mm_sql") = 3
                                Label10.Text = "ZAKŁADKA AUTOMAGAZYNOWANIE TOWARU WARIANT 3"
                            Else
                                sqlexp = ""
    ...
    Protected Sub BTowarZlozenie_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BTowarZlozenie.Click
        ...
        Using conn As New OracleConnection(dajarSWMagazyn_MIA.MyFunction.connString)
            ...
            Else
                ...
                adres_zlozenia = hala & rzad & regal & polka
            End If
        ...
        Else
            ...
            For Each row As GridViewRow In GridViewAdresy.Rows
                ...
                If cb_row IsNot Nothing And cb_row.Checked Then
                    ...
                    RefreshDDLRegal(DDLRegal, DDLHala.SelectedValue, DDLRzad.SelectedValue)
                    DDLRegal.SelectedValue = ob_mag.adres.Substring(3, 2).ToString
                    RefreshDDLPolka(DDLPolka, DDLHala.SelectedValue, DDLRzad.SelectedValue, DDLRegal.SelectedValue)
                    DDLPolka.SelectedValue = ob_mag.adres.Substring(5, 2).ToString
                    ''DDLStrefaMagazyn.SelectedValue = ob_mag.strefa.ToString
