...
Partial Public Class a_pakowanie
    ...
    Public Sub LadujDaneGridViewDyspozycje(ByVal filtr As String)
        Using conn As New OracleConnection(dajarSWMagazyn_MIA.MyFunction.connString)
            ...
            If filtr <> "" Then
                sqlexp = "select rownum as lp, z.*,(select to_char(autodata,'RRRR/MM/DD') data_mag from dp_swm_mia_oper where status=z.nr_zamow and typ_oper like 'MG%') data_mag from (" _
                ...
                & " zg.nr_zamow = dm.nr_zamow and zg.schemat = dm.schemat and dm.status in('PA')" _
                & " union all" _
                ...
                & " zg.nr_zamow = dm.nr_zamow and zg.schemat = dm.schemat and dm.status not in('PE','RW')" _
    ...
    Public Sub AktualizujGridViewDyspozycjeStatusy()
        If GridViewDyspozycje.Rows.Count > 0 Then
            For Each row As GridViewRow In GridViewDyspozycje.Rows
                ...
                ''Dim etykieta_id As String = row.Cells(11).Text.ToString
