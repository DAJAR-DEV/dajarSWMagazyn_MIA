...
from (
select db.skrot,dz.nr_zamow,dz.schemat,dz.stat_u1 from dp_swm_lub_zog dz,dp_swm_lub_buf db where dz.nr_zamow=db.nr_zamow and dz.schemat=db.schemat and dz.stat_u1 in('DR','RP') 
order by to_char(dz.stda_u1,'YYYY-MM-DD'), decode(dz.stat_u1, 'MG', 1, 'RP', 2, 'DR', 3)
) w 
) w1 where w1.ile_z = 0 and w1.ile_np > 0
) w2 left join dp_swm_lub_towary dt on w2.skrot = dt.skrot and dt.strefa in('N','P') and dt.is_active='1' order by w2.skrot, decode(dt.strefa, 'N', 1, 'P', 2)
