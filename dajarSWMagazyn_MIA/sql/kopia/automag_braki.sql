select distinct w3.skrot,r.nazdop,r.jm,r.kod_tow,r.skt2_0,r.klasa, (select max(to_char(autodata, 'YYYY/MM/DD HH24:MI:SS')) from dp_swm_towary_hist where oper='OP_BR' and skrot=w3.skrot) data_brak from (
...
from (
select dt.skrot,dt.adres,dt.strefa from dp_swm_towary_hist dt where dt.oper='OP_BR'
) w 
) w1 where w1.ile_z = 0 and w1.ile_np > 0
) w2 where w2.skrot not in(
select distinct (w.skrot) from (
select db.skrot from dp_swm_zog dz,dp_swm_buf db where dz.nr_zamow=db.nr_zamow and dz.schemat=db.schemat and dz.stat_u1 in('DR','RP') 
