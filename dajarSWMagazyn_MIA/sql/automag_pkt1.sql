...
(select substr(typ_oper,0,2) from dp_swm_lub_oper where status=dz.nr_zamow and typ_oper like '%['||dz.schemat||']%' and (typ_oper like 'DR[%' or typ_oper like 'RP[%')) typ_oper
from dp_swm_lub_zog dz,dp_swm_lub_buf db where dz.nr_zamow=db.nr_zamow and dz.schemat=db.schemat and dz.stat_u1 in('MG') 
...
) w2 left join dp_swm_lub_towary dt on w2.skrot = dt.skrot and dt.strefa in('N','P') and dt.is_active='1' order by decode(dt.strefa, 'N', 1, 'P', 2), w2.skrot
